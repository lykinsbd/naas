...
def main() -> None:
    ...
    # Replace unstable header (temp filename + timestamp) with fixed comment
    content = OUTPUT_PATH.read_text()
    lines = content.splitlines(keepends=True)
    stable_header = "# generated by datamodel-codegen from NAAS OpenAPI spec\n\n"
    # Skip the comment block (lines starting with #) and the blank line after
    start = 0
    for i, line in enumerate(lines):
        if not line.startswith("#") and line.strip() != "":
            start = i
            break
    OUTPUT_PATH.write_text(stable_header + "".join(lines[start:]))
