...
from naas.library.auth import tacacs_auth_lockout, report_tacacs_failure, Credentials
...
class TestTacacsAuthLockout:
    ...
    def test_old_failures_expire(self, fake_redis, monkeypatch):
        ...
        fail_dict = {
            "failure_count": 9,
            "failure_timestamps": dumps(old_timestamps)
        }
...
class TestReportTacacsFailure:
    ...
    def test_report_increments_count(self, fake_redis):
        ...
        report_tacacs_failure(
            username="testuser",
            existing_fail_count=1,
            existing_fail_times=existing_times,
            redis=fake_redis
        )
